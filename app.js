...
function showStory(index) {
  ...
  el('category').textContent = s.category.toUpperCase();
  ...
  el('ticker').textContent = s.ticker || 'Peace • Love • Truth • Compassion • Respect • Help one another';
...
function runAd(done) {
  playing = false;
  const ad = ads.length ? ads[Math.floor(Math.random() * ads.length)] : {
...
loadData().catch(err => {
  ...
  el('summary').textContent = 'Check that all files were uploaded to the same GitHub repository.';
});
